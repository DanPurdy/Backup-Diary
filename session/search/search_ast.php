...
if(!empty($_POST['astSearch'])){
    $session = "%".$_POST['astSearch']."%";
}else{
    $session= "%"."'%'"."%";
};
...
try{
    $sth=$dbh->prepare("SELECT session.*, client.cliName, composer.cmpName, fixer.fixName, project.prjName, assistant.astName,engineer.engName
                        ...
                        WHERE assistant.astName LIKE :astName
                        ORDER BY sessDate DESC;");
    
    $sth->bindParam(':astName', $session, PDO::PARAM_STR);
    ...
    $sth=$dbh->prepare("SELECT session.*, client.cliName, composer.cmpName, fixer.fixName, project.prjName, assistant.astName,engineer.engName
                        ...
                        INNER JOIN composer ON session.cmpID=composer.cmpID
                        INNER JOIN fixer ON session.fixID=fixer.fixID
                        WHERE assistant.astName LIKE :astName AND (sessDate BETWEEN :dateS AND :dateE)
                        ORDER BY sessDate DESC;");
...
?>
        ...
        <div id="Results">
            <div id="subHead"><h3>Search By Assistant: <?=htmlentities($_POST['astSearch']);?></h3></div>
