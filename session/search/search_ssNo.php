...
if(!empty($_POST['ssNoSearch'])){
    $session = $_POST['ssNoSearch'];
}else{
    $session= 100;
};
...
try{
    $sth=$dbh->prepare("SELECT session.*, client.cliName,composer.cmpName, fixer.fixName, project.prjName, assistant.astName,engineer.engName
                        ...
                        WHERE session.ssNo = :ssNo
                        ORDER BY session.sessDate DESC; ");
    
    $sth->bindParam(':ssNo', $session, PDO::PARAM_INT);
  ...
  ?>
        ...
        <div id="Results">
            <div id="subHead"><h3>Session Sheet #: <?=htmlentities($_POST['ssNoSearch']);?></h3></div>
