...
try{
    $sth=$dbh->prepare("SELECT session.*, client.cliName, composer.cmpName, fixer.fixName, project.prjName, assistant.astName,engineer.engName
                        ...
                        WHERE composer.cmpName LIKE :cmpName
                        ORDER BY sessDate DESC;");
    
    $sth->bindParam(':cmpName', $session, PDO::PARAM_STR);
  ...
  ?>
        ...
        <div id="Results">
            <div id="subHead"><h3>Results for Composer Name: <?=htmlentities($_POST['cmpSearch']);?></h3></div>
