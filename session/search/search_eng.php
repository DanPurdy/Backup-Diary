...
if(!empty($_POST['engSearch'])){
    $session = "%".$_POST['engSearch']."%";
}else{
    $session= "%"."'%'"."%";
}
...
try{
    $sth=$dbh->prepare("SELECT session.*, client.cliName, composer.cmpName, fixer.fixName, project.prjName, assistant.astName, engineer.engName
                        ...
                        WHERE engineer.engName LIKE :engName
                        ORDER BY sessDate DESC;");
    
    $sth->bindParam(':engName', $session, PDO::PARAM_STR);
    ...
    $sth=$dbh->prepare("SELECT session.*, client.cliName, composer.cmpName, fixer.fixName, project.prjName, assistant.astName,engineer.engName
                        ...
                        INNER JOIN composer ON session.cmpID=composer.cmpID
                        INNER JOIN fixer ON session.fixID=fixer.fixID
                        WHERE engineer.engName LIKE :engName AND (sessDate BETWEEN :dateS AND :dateE)
                        ORDER BY sessDate DESC;");
...
?>
        ...
        <div id="Results">
            <div id="subHead"><h3>Search By Engineer: <?=htmlentities($_POST['engSearch']);?></h3></div>
