...
if($_POST['sessCont']!=0 OR !empty($_POST['sessCont'])){
    ...
    $st1 = $dbh->prepare('INSERT INTO backup (bakName) 
    VALUES (NULL);' );
    
    $st1->execute();
        
    $bakID = $dbh->lastInsertID('bakID');
}
try{
$sth = $dbh->prepare('INSERT INTO session (stdID, cliID, prjID, fixID, engID, astID, ssNo, sessDate, startTime, endTime, cmpID, bakID) 
...
$sth->execute();
$id = $dbh->lastInsertId('sessID');


header('Location: /session/new_session.php');
