...
<div id="micWarning" class="Eassigned"><h3>Sorry Microphone <?php echo htmlentities($_GET['micNo']);?> is already signed out on <a href="edit_microphone.php?sesID=<?php echo $errorSess;?>&remove=1">Session # <?php echo $errorSess;?></a></h3></div>
    
    <?php }elseif($e == 2){ ?>
       <div id="micWarning"><h3>Sorry! Microphone <?php echo htmlentities($_GET['micNo']);?> is already signed into the cupboard. Was it signed out properly?</h3></div>
       <?php }elseif ($e==3) { ?>
           <div id="micWarning" class="REassigned"><h3>Sorry! Microphone <?php echo htmlentities($_GET['micNo']);?> is assigned to <a href="edit_microphone.php?sesID=<?php echo $errorSess;?>&remove=1">Session # <?php echo $errorSess;?> You must return it there.</a></h3></div>

               
         <?php  }elseif($e==4) { ?>
             <div id="micWarning"><h3>You must choose a session to transfer mics to from the dropdown menu.</h3></div>
      <?php   }elseif($e==5){ ?>
             <div id="micWarning"><h3>Sorry! Microphone <?php echo htmlentities($_GET['micNo']); ?> is currently out for repair.</h3></div>
   <?php   } ?>
...
<div class="microphoneAssigned">
   <div class="backupDriveTitle"><h3>Microphones</h3></div> 
    ...
    <div class="micNo"><?php echo $row2['micID'] . " " .$row2['micMake'] . " " .$row2['micModel'] ?></div>
...
<form id="micForm" action="returnMic.php" method="post"> 
    ...
    <div class="microphoneAssigned">
        ...
        if($microphone->count >=1){?><div class="micNo"><h3><input type="checkbox" class="checkall"> Check All</div></h3><br> <?php } ?>
            <?php
     foreach($micList as $row2) {  
        
          ?>

    <div class="micNo"><input type="checkbox" name="micNo_check[]" value="<?php echo$row2['micID'];?>"/> <?php echo $row2['micID'] . " &nbsp;|&nbsp; " .$row2['micMake'] . " " .$row2['micModel'] ?></div>
